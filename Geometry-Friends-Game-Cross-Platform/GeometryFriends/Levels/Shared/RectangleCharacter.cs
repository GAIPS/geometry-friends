...
namespace GeometryFriends.Levels.Shared
{
    internal class RectangleCharacter : GeoShape
    {
        ...
        private void SwapWidthAndHeightAccordingToOrientation()
        {
            ...
            if (Math.Abs(angle) < changeOrientationAngle || Math.Abs(Math.PI - Math.Abs(angle)) < changeOrientationAngle || Math.Abs((Math.PI / 4) + angle) < changeOrientationAngle) // swap width and height            
            {
                float temp = height;
                height = width;
                width = temp;
